...
@onready var btn_1 = $"../Control/TouchScreenButton"
@onready var btn_2 = $"../Control/TouchScreenButton3"
@onready var btn_3 = $"../Control/TouchScreenButton2"
@onready var btn_4 = $"../Control/TouchScreenButton4"
...
func handle_jumping():
	if jumping_enabled:
		if continuous_jumping: # Hold down the jump button
			if Input.is_action_pressed(JUMP) or btn_1.is_pressed() and is_on_floor() and !low_ceiling:
				if jump_animation:
					JUMP_ANIMATION.play("jump", 0.25)
				velocity.y += jump_velocity # Adding instead of setting so jumping on slopes works properly
		else:
			if Input.is_action_just_pressed(JUMP) or btn_1.is_pressed() and is_on_floor() and !low_ceiling:
				if jump_animation:
					JUMP_ANIMATION.play("jump", 0.25)
...
func handle_state(moving):
	if sprint_enabled:
		if sprint_mode == 0:
			if Input.is_action_pressed(SPRINT) or btn_2.is_pressed() and state != "crouching":
				if moving:
					if state != "sprinting":
						enter_sprint_state()
				else:
			...
			if moving:
				...
				if Input.is_action_pressed(SPRINT) or btn_2.is_pressed() and state == "normal":
					enter_sprint_state()
				if Input.is_action_just_pressed(SPRINT) or btn_2.is_pressed():
					match state:
						"normal":
							enter_sprint_state()
						"sprinting":
	...
	if crouch_enabled:
		if crouch_mode == 0:
			if Input.is_action_pressed(CROUCH) or btn_3.is_pressed() and state != "sprinting":
				if state != "crouching":
					enter_crouch_state()
			elif state == "crouching" and !$CrouchCeilingDetection.is_colliding():
				enter_normal_state()
		elif crouch_mode == 1:
			if Input.is_action_just_pressed(CROUCH) or btn_3.is_pressed():
				match state:
					"normal":
						enter_crouch_state()
					"crouching":
